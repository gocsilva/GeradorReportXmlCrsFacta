...
class DataPreparationService:
    ...
    def _pending_rows(self, prepared: PreparedData) -> list[dict[str, object]]:
        ...
        rows.append({"linha_planilha": "", "cliente_mascarado": "", "documento_brasileiro_mascarado": "", "numero_conta_mascarado": "", "classificacao_fatca": "CRS", "us_tax_id_informado": "", "status": "CRS_V2_PJ_IN", "motivo_ausencia": "Titular PJ CRS v2 usa CNPJ em IN issuedBy=BR conforme XSD v2.", "tratamento_aplicado": "ReportingFI CRS permanece com IN issuedBy=KY; AccountHolder/Organisation PJ usa IN issuedBy=BR no schema v2.", "elemento_omitido_ou_gerado": "IN", "resultado_validacao_xsd": "validado_apos_geracao", "bloqueante_para_envio": "nao", "data_hora": datetime.now().replace(microsecond=0).isoformat(sep=" ")})
        return rows
